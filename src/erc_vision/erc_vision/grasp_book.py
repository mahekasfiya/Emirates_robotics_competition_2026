...
Picks the target book off the shelf
"""
...
class GraspBook(Node):
    ...
    def move_torso(self, value, wait=True):
        ...
        wall_deadline = time.time() + (self.TORSO_SETTLE_SEC + 6.0) * 5
        last = None
        while time.time() < wall_deadline:
            ...
            if use_sim:
                ...
                # Give up early only after a *2 s* stall.
                if elapsed > self.TORSO_SETTLE_SEC + 2.0:
    ...
    def set_gripper(self, value, wait=True, duration=None):
        ...
        if wait:
            self.sleep_sim(self.GRIP_MOVE_SEC + 0.3)
    ...
    def drive_to(self, goal_xy, goal_yaw, label=''):
        ...
        wall_cap = time.time() + self.ALIGN_TIMEOUT * 10
        settled = None
        best = (1e9, 1e9)
    ...
    def redetect_book(self, colour, expected_map_xyz):
        started = self.sim_now()
        wall_cap = time.time() + self.DETECT_TIMEOUT_SEC * 10
        best = None
        samples = []
        last_stamp = None
        frames = 0
    ...
    def recover_after_failure(self):
        ...
        # Tuck: 3.5 s trajectory + 4.0 s settle.
        self.send_traj(self.arm_pub, ARM_JOINTS, RIGHT_TUCK, 3.5)
        self.sleep_sim(4.0)
        self.get_logger().info(
