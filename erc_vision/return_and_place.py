...
Frame handling:
  Latest-frame cache. The robot is stationary while it looks for the bin,
  so the most recent frame from each camera is what we want. At low sim
  real-time factor the ApproximateTimeSynchronizer silently dropped nearly
  every color/depth pair because their stamps drifted apart.
"""
...
class ReturnAndPlace(Node):
    ...
    def __init__(self):
        ...
        # The bin's settled pose is world (-1.00, 0.00, 0.845), which is 1.0 m
        # straight ahead of the start zone once the robot faces the table.
    ...
    def sim_now(self):
        """Seconds on the node clock, falling back to the wall clock before the
        first /clock message arrives. Waits timed on time.time() give up while
        a joint is still travelling, because the real-time factor here is well
        below 1."""
        t = self.get_clock().now().nanoseconds / 1e9
        return t if t > 0 else time.time()

    def move_torso(self, value, wait=True):
        """Command the torso and wait for /joint_states to confirm arrival.
        The arm solutions here all sit near the torso's limit, and move_arm()
        publishes only the arm joints - an unfinished torso move would offset
        the entire descent into the bin with no error raised."""
        ...
        if not wait:
            return True
        start = self.sim_now()
        wall_cap = time.time() + (self.TORSO_SETTLE_SEC + 6.0) * 25
        last = None
        while time.time() < wall_cap:
            ...
            if actual is not None and abs(actual - value) < self.TORSO_TOLERANCE:
                ...
                return True
            if self.sim_now() - start > self.TORSO_SETTLE_SEC + 6.0:
        ...
        while time.time() < deadline:
            ...
            for cnt in sorted(contours, key=cv2.contourArea, reverse=True):
                ...
                if not self.MIN_BIN_DEPTH <= med <= self.MAX_BIN_DEPTH:
                    continue
                # np.nonzero returns pixels in row-major order, so taking the
                # FIRST 400 takes only the top rows of the blob - the far side
                # of the bin - and drags the centroid away from the robot.
                # That is what put the bin at map y=1.46 when it actually sits
                # at 1.00. Sample the whole mask evenly instead.
                ...
                if len(pts) < 50:
                    continue
                arr = np.array(pts)
                # Median, not mean: the rim, the inner walls and the floor are
                # all in this cloud and they are not symmetrically distributed.
    ...
    def attempt_place(self):
        ...
        # Cross-check vision against where the bin has to be. The nominal pose
        # comes from the world: the bin sits 1.0 m in front of the start zone.
        # A fix further out than BIN_TRUST_RADIUS is a bad blob, not a moved bin.
        ...
        # Stand OFF from the bin along the heading, and offset sideways so the
        # bin lines up with the right shoulder. The previous version put the
        # base ON the bin and then measured `forward` as a purely lateral
        # offset, so it always came out near zero and the reach limit never
        # applied - only the MAX_BASE_Y clamp kept it sane by accident.
        ...
        # If the hover will not solve at the bin centre, aim nearer its close
        # rim. The bin is 0.31 m deep and the book only 0.16 m, so pulling the
        # aim point in still lands the book fully inside.
        q_hover = None
        ...
        # Every hover solution here wants the torso near its limit, and
        # move_arm() publishes ONLY the arm joints. Command the torso first
        # and wait, then re-solve against the height actually reached -
        # otherwise the whole descent runs offset by the shortfall.
        ...
        self.move_arm([q_hover], seconds_each=3.5)
        ...
        qs_up, ok = self.kin.cartesian_path(qs[-1] if ok else q_hover,
                                            [hover_p], R)
        if ok:
            self.move_arm(qs_up, seconds_each=2.0)
        return placed
