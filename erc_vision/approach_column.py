...
TWO STAGES, ON PURPOSE
  Stage A - Nav2 NavigateToPose gets the robot across the arena with
            obstacle avoidance and replanning.  Nav2's goal checker is a
            tolerance ("close enough"), not a controller, so it will
            happily declare success up to xy_goal_tolerance away and at
            yaw_goal_tolerance off-square.
  Stage B - a short closed-loop alignment on the map->base_link transform
            drives the remaining error to a couple of centimetres and
            about half a degree, using the mecanum base's ability to
            strafe.  This is what makes "exactly perpendicular, right in
            front" true rather than approximately true, and it is also
            what the grasp stage will need.

WHERE THE GOAL COMES FROM
  column_detector.py wrote the target marker's map-frame position AND the
  shelf face normal (fitted through all visible markers).  The goal is
  simply:
      position = marker + standoff * normal
      heading  = -normal              (i.e. pointing at the shelf)
  No relative-bearing arithmetic, no assumption about which way the robot
  happened to be facing when perception finished.

STANDOFF SANITY
  base_link's collision box is 0.717 x 0.497 m, so the front edge sits
  0.36 m ahead of base_link's origin.  The shelf face is ~0.10 m behind
  the marker plane.  At the default 0.55 m standoff the base stops with
  roughly 0.29 m of clearance to the shelf.  Reduce the standoff for a
  longer arm reach only if you have re-checked that number.
"""
...
class ApproachColumn(Node):
    ...
    ALIGN_RATE = 20.0             # Hz
    # If Nav2 left us further away than this, something went wrong upstream.
    # Creeping several metres under an uncapped P controller with no costmap
    # in the loop is exactly how you drive into furniture, so refuse instead.
    ...
    def __init__(self):
        ...
        self.declare_parameter('standoff_override', 0.0)   # 0 = use the JSON value
    ...
    def _await(self, future, timeout_sec):
        """rclpy Futures do not take a timeout and do not block on their own -
        they are not concurrent.futures.  A background thread is spinning
        this node (see main), so poll done() and only then call result()."""
    ...
    def run(self):
        ...
        if not self.send_nav_goal(goal_xy, goal_yaw):
            # Nav2 failing does not necessarily mean we are nowhere useful;
            # fall through to alignment, which will refuse if we are too far.
            self.exit_code = 1
    ...
    def precision_align(self, goal_xy, goal_yaw):
        """Closed-loop on map->base_link.  Errors are rotated into the body
        frame and issued as simultaneous x / y / yaw commands, which the
        mecanum base can execute directly - no back-and-forth three-point
        turn, and no coupling between fixing the heading and fixing the
        position."""
...
def main(args=None):
    ...
    node = ApproachColumn()
    # run() blocks on TF lookups and action-client waits, both of which need
    # incoming messages processed concurrently, so spin on a background thread.
